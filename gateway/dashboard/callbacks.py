...
def register(app: dash.Dash) -> None:
    ...
    @app.callback(
        Output(L.COUGH_EVENT_DATE_ID, "options"),
        Output(L.COUGH_EVENT_DATE_ID, "value"),
        Input(L.CLIENT_DROPDOWN_ID, "value"),
        Input(L.POLL_INTERVAL_ID, "n_intervals"),
        State(L.COUGH_EVENT_DATE_ID, "value"),
    )
    def update_event_dates(client_id, _n, selected_date):
        if not client_id:
            return [], None
        payload = api_get(
    ...
    @app.callback(
        ...
        Input(L.CLIENT_DROPDOWN_ID, "value"),
        Input(L.COUGH_EVENT_DATE_ID, "value"),
    )
    ...
    def update_patient(_n, client_id, range_mode):
        is_7d = range_mode == "7d"
        total_label = (
            "Cough bouts — last 7 completed days"
            if is_7d
            else "Cough bouts — last 24 hours"
        )
        if not client_id:
            return (
        ...
        total = stats.get("last_7d_count" if is_7d else "last_24h_count")
        return (
...
def _count_figure(stats: dict, range_mode: str, client_id: str) -> go.Figure:
    return _seven_day_figure(stats, client_id) if range_mode == "7d" else _rolling_24h_figure(stats, client_id)


def _coarse_tick_step(maximum: int | float) -> int:
    if maximum <= 10:
        return 2
    if maximum <= 30:
        return 5
    if maximum <= 60:
        return 10
    magnitude = 10 ** max(len(str(int(maximum))) - 2, 1)
    return max(int(math.ceil(maximum / 6 / magnitude) * magnitude), 10)
...
def _rolling_24h_figure(stats: dict, client_id: str) -> go.Figure:
    ...
    valid_points, x_values, totals, custom = [], [], [], []
    for point in points:
        start = _parse_datetime(point.get("ts"))
        if start is None:
            continue
        display_end = start + timedelta(minutes=30) - timedelta(seconds=1)
        ...
        totals.append(total)
        custom.append([f"{start:%H:%M}–{display_end:%H:%M}", dry, wet, unknown])
    ...
    for cough_type in ("dry", "wet", "unknown"):
        fig.add_bar(
            ...
            hovertemplate=(
                ...
                "Dry: %{customdata[1]}<br>"
                "Wet: %{customdata[2]}<br>"
                "Unknown: %{customdata[3]}<extra></extra>"
            ),
        )
    fig.add_scatter(
        x=x_values,
...
def _seven_day_figure(stats: dict, client_id: str) -> go.Figure:
    ...
    for period in ("day", "night"):
        custom = []
        for point in points:
            period_types = point.get(f"{period}_types") or {}
            custom.append(
                [
                    int(point.get(period, 0)),
                    int(period_types.get("dry", 0)),
                    int(period_types.get("wet", 0)),
                    int(period_types.get("unknown", 0)),
                ]
            )
        fig.add_bar(
            x=x_values,
    ...
    if baseline.get("available") and baseline.get("baseline") is not None:
        ...
        edge = timedelta(days=0.34)
        fig.add_scatter(
            x=[x_values[0] - edge, x_values[-1] + edge],
            y=[value, value],
            mode="lines",
            showlegend=False,
        ...
        fig.add_annotation(
            x=x_values[-1] + edge,
            y=value,
            text=f"Personal baseline {value:.1f}/day",
            showarrow=False,
            xanchor="right",
            yanchor="bottom",
...
def _baseline_content(status: dict):
    ...
    if not status.get("available"):
        return html.Div(
            ...
            children=[
                html.Strong("Calibrating Personal EWMA"),
                html.P(
                    f"{status.get('warmup_remaining', 7)} more completed observed day(s) required.",
    ...
    title = (
        "Above Personal EWMA threshold"
        if status.get("above_baseline")
        else "Within Personal EWMA threshold"
    )
    return html.Div(
        className="finding finding-active" if status.get("above_baseline") else "finding finding-stable",
        children=[
            html.Strong(title),
            html.Div(
                className="finding-values",
                children=[
                    _finding_value("Current 24h", status.get("c24")),
                    _finding_value("Personal EWMA baseline", f"{status['baseline']:.1f}"),
...
def _treatment_content(status: dict):
    ...
    if not status.get("available"):
        ...
        messages = {
            "warmup": "The first evaluation week is still in progress.",
            "awaiting_completed_comparison_week": "The next completed week will provide the first comparison.",
            "reference_warmup": "Not enough observed days existed before the evaluation week.",
            "incomplete_monitoring": (
                f"Only {status.get('evaluation_observed_days', 0)}/7 observed days are available; no conclusion is shown."
            ),
            "zero_reference": "The EWMA reference is unavailable for this week.",
        }
        return html.Div(
            className="finding finding-warmup",
            children=[
                html.Strong(f"Week {current_week} in progress"),
                html.P(
                    f"{current_range}. {messages.get(reason, 'No valid comparison is available yet.')}",
                    className="finding-copy",
                ),
            ],
        )
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    _finding_value("EWMA reference", f"{status['ewma_reference']:.1f}/day"),
                    _finding_value(f"Week {evaluation_week} average", f"{status['current']:.1f}/day"),
                    _finding_value("Change", f"{change:+.1f}%" if change is not None else "—"),
                ],
            ),
            html.P(
                f"Week {current_week} is in progress ({current_range}); partial data is not interpreted. "
                "The completed-week comparison is descriptive and does not attribute change to treatment.",
                className="finding-copy",
            ),
        ],
    )
...
def _empty_baseline():
    return html.P("Select a patient to evaluate the Personal EWMA.", className="empty-state")


def _empty_treatment():
    return html.P("Select a patient to evaluate treatment-week trends.", className="empty-state")
